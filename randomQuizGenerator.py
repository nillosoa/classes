...
# A bit of me(probably not the best way though)
fname_wo_ext = sys.argv[0].split('/')[-1].replace('.py', '')
cwd = path.dirname(path.abspath(__file__))
if not path.isdir(path.join(cwd, fname_wo_ext)):
    mkdir(path.join(cwd, fname_wo_ext))
...
for quizNum in range(35):
    ...
    quizFile = open(path.join(cwd, fname_wo_ext, 'capitalsquiz%s.txt' %(quizNum + 1)), 'w')
    answersKeyFile = open(path.join(cwd, fname_wo_ext, 'capitalquiz_answers%s.txt' %(quizNum + 1)), 'w')
